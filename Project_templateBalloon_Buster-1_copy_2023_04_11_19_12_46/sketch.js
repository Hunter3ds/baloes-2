...
var imagemArco, imagemFlecha, imagem_balaoVerde, imagem_balaoVermelho, imagem_balaoRosa ,imagem_balaoAzul, imagemPlanoFundo, pontuacao;

var pontuacao = 0;

var grupoDeBaloes, grupoDeFlechas;
...
function setup() {
  ...
  baloesVermelhos = new Group();
  baloesAzuis = new Group();
  baloesVerdes = new Group();
  baloesRosas = new Group();
...
function draw() {
 background(0);
  ...
  if (keyDown("space")) {
   ...
   if(grupoDeFlechas.isTouching(baloesVermelhos)){
    baloesVermelhos.destroyEach();
    grupoDeFlechas.destroyEach();
    pontuacao = pontuacao + 1
   }
   if(grupoDeFlechas.isTouching(baloesAzuis)){
    baloesAzuis.destroyEach();
    grupoDeFlechas.destroyEach();
    pontuacao = pontuacao + 2;
   }
   if(grupoDeFlechas.isTouching(baloesVerdes)){
    baloesVerdes.destroyEach();
    grupoDeFlechas.destroyEach();
    pontuacao = pontuacao + 3;
   }
   if(grupoDeFlechas.isTouching(baloesRosas)){
    baloesRosas.destroyEach();
    grupoDeFlechas.destroyEach();
